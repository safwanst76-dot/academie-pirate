// ═══════════════════════════════════════════════════════════════
// AUDIO.JS — 🔮 Namek · Univers : Jujutsu Kaisen
// ✅ Supabase Storage bucket island-namek/music/ — UNIQUEMENT
// ❌ YouTube supprimé — règle AU-01
// ✅ Isolation préfixe jjk-* — règle AU-02
...
(function () {
  ...
  var STORAGE_BASE = 'https://bwxzrqsvccqmzvonsswi.supabase.co/storage/v1/object/public/island-namek/music/';

  // Mapping track-id → fichier MP3 dans le bucket
  var SUPABASE_FILES = {
    'jjk-map':     'jjk-map.mp3',
    'jjk-battle':  'jjk-battle.mp3',
    'jjk-boss':    'jjk-boss.mp3',
    'jjk-isle':    'jjk-isle.mp3',
    'jjk-victory': 'jjk-victory.mp3',
  };
  ...
  function _play(url) {
    ...
    if (p && typeof p.then === 'function') {
      p.then(function () {
        _currentSrc = url;
        console.log('[JJK Patch] ✓ Audio Supabase :', url.split('/').pop());
      }).catch(function (e) {
        if (e && e.name === 'NotAllowedError') {
          console.info('[JJK Patch] autoplay bloqué, attente clic');
        ...
        console.warn('[JJK Patch] Erreur Supabase :', e && e.message);
      });
    } else {
      _currentSrc = url;
    }
  ...
  window.playBGM = function (track) {
    var isJjk = (typeof track === 'string') && track.startsWith('jjk-');
    ...
    if (!file) {
      console.warn('[JJK Patch] Track non mappé :', track);
      return;
    }
  ...
  console.info('🔮 audio-namek.js v1 — Supabase uniquement, préfixe jjk-*, YouTube supprimé');
