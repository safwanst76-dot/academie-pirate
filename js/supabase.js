...
async function sbGetParentProfile(userId) {
  const { data } = await sb.from('profiles_parents').select('*').eq('id', userId).single();
  return data;
}
...
async function handleSignedIn(user) {
  ...
  // 2. Vérifier si c'est un enfant connu
  ...
  // 3. Nouvel utilisateur → choisir rôle
  showRoleChoice(user);
}

// ═══════════════════════════════════════
// CHOIX DU RÔLE (nouvel utilisateur)
// ═══════════════════════════════════════
...
function startChildOnboarding(userId, email) {
  ...
  if (avatarScreen) {
    ...
    // Patch saveAvatar pour persister en DB
...
function startParentOnboarding(userId, email) {
  ...
  const fieldStyle = `
    background:rgba(255,255,255,.07);border:2px solid rgba(59,130,246,.3);
  ...
  screen.innerHTML = `
    <div style="font-family:'Bangers',cursive;font-size:2rem;color:#3b82f6;letter-spacing:3px;text-align:center;margin-top:20px;">
      ⚓ PROFIL CAPITAINE
    </div>
    <div style="font-family:'Nunito',sans-serif;color:rgba(255,255,255,.45);font-size:.85rem;text-align:center;max-width:340px;">
      Ces informations nous permettent de personnaliser l'expérience et de gérer l'abonnement
    </div>
    <div style="
      background:rgba(0,0,0,0.5);border:2px solid rgba(59,130,246,.25);
      border-radius:20px;padding:28px;width:min(420px,92vw);
      display:flex;flex-direction:column;gap:16px;box-sizing:border-box;
    ">
      <div style="display:flex;flex-direction:column;gap:6px;">
        <label style="${labelStyle}">👤 Prénom</label>
        <input id="parentPrenom" type="text" placeholder="Ton prénom" style="${fieldStyle}">
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;">
        <label style="${labelStyle}">👤 Nom</label>
        <input id="parentNom" type="text" placeholder="Ton nom de famille" style="${fieldStyle}">
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;">
        <label style="${labelStyle}">📱 Téléphone <span style="color:rgba(255,255,255,.3)">(optionnel)</span></label>
        <input id="parentPhone" type="tel" placeholder="+33 6 XX XX XX XX" style="${fieldStyle}">
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;">
        <label style="${labelStyle}">📧 Email</label>
        <input type="email" value="${email}" readonly style="${fieldStyle}opacity:.5;cursor:not-allowed;">
      </div>
      <button id="parentSaveBtn" onclick="saveParentProfile('${userId}','${email}')" style="
...
async function saveParentProfile(userId, email) {
  ...
  const { error } = await sb.from('profiles_parents').upsert({
    id: userId, email, prenom, nom, phone: phone || null
  });
...
async function showParentDashboard(userId, parentProfile) {
  ...
  // Charger enfants + progression
  const { data: enfants } = await sb.from('profiles')
    .select('*, progression(*)')
    .eq('parent_id', userId);
  ...
  dash.style.cssText = `
    display:flex;flex-direction:column;align-items:center;
    gap:24px;padding:30px 20px;min-height:80vh;
  `;
  ...
  const cardsHTML = enfants && enfants.length
    ? enfants.map(e => {
        ...
        return `
          <div style="
            background:rgba(0,0,0,0.45);border:2px solid rgba(255,215,0,.2);
            border-radius:16px;padding:20px;display:flex;flex-direction:column;
            gap:12px;min-width:220px;max-width:300px;flex:1;
          ">
            <div style="display:flex;align-items:center;gap:12px;">
              <img src="assets/images/avatars/${e.avatar_id||'luffy'}.png"
                style="width:52px;height:52px;border-radius:50%;border:2px solid #ffd700;object-fit:cover;"
                onerror="this.style.display='none'">
              <div>
                <div style="font-family:'Bangers',cursive;font-size:1.2rem;color:#ffd700;">${e.username||'Pirate'}</div>
                <div style="font-family:'Nunito',sans-serif;font-size:.75rem;color:rgba(255,255,255,.4);">
                  ${e.date_naissance ? new Date(e.date_naissance).toLocaleDateString('fr-FR') : 'Âge non renseigné'}
                </div>
              </div>
            </div>
            <div style="font-family:'Nunito',sans-serif;font-size:.82rem;color:rgba(255,255,255,.6);">
              📚 Îles complétées : <strong style="color:#ffd700">${completed} / 8</strong>
            </div>
            <div style="font-family:'Nunito',sans-serif;font-size:.82rem;color:rgba(255,255,255,.6);">
              ⭐ XP total : <strong style="color:#ffd700">${xpTotal}</strong>
            </div>
          </div>`;
      }).join('')
    : `<div style="font-family:'Nunito',sans-serif;color:rgba(255,255,255,.35);font-size:.9rem;text-align:center;padding:20px;">
        Aucun enfant lié pour l'instant.<br>
        <span style="font-size:.8rem;">Ton enfant doit renseigner ton email lors de son inscription.</span>
      </div>`;

  dash.innerHTML = `
    <div style="font-family:'Bangers',cursive;font-size:2.2rem;color:#3b82f6;letter-spacing:3px;text-align:center;">
      👨‍👩‍👧 TABLEAU DE BORD CAPITAINE
    </div>
    <div style="
      background:rgba(0,0,0,0.4);border:2px solid rgba(59,130,246,.25);
      border-radius:16px;padding:20px 24px;width:min(480px,90vw);box-sizing:border-box;
    ">
      ...
      <div style="font-family:'Nunito',sans-serif;color:rgba(255,255,255,.45);font-size:.85rem;margin-top:4px;">
        📧 ${parentProfile.email}
        ${parentProfile.phone ? `<br>📱 ${parentProfile.phone}` : ''}
      </div>
    </div>
    <div style="font-family:'Bangers',cursive;font-size:1.4rem;color:#ffd700;letter-spacing:2px;">🧒 MES AVENTURIERS</div>
    <div style="display:flex;flex-wrap:wrap;gap:16px;justify-content:center;width:100%;max-width:700px;">
      ${cardsHTML}
    </div>
    <button onclick="sbSignOut()" style="
      font-family:'Bangers',cursive;font-size:1rem;letter-spacing:2px;
      padding:10px 24px;border-radius:12px;border:2px solid rgba(255,255,255,.15);
      background:transparent;color:rgba(255,255,255,.35);cursor:pointer;margin-top:8px;
