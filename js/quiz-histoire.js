// ═══════════════════════════════════════════════════════════
// QUIZ-HISTOIRE.JS v2 — Académie Pirate
// Matière : HISTOIRE — 6ème / 5ème — Univers Dragon Ball Z
// ✅ Fond animé DBZ (strips comme One Piece)
// ✅ Avatars locaux assets/images/dbz/
// ✅ Musique DBZ YouTube
// ✅ Cinématique plein écran avec image perso
// ✅ Contraste quiz renforcé
// ═══════════════════════════════════════════════════════════
...
// ── Musique DBZ YouTube ──
var HIST_BGM = {
  map:     'HnGBMgTNf9Y',  // DBZ Main Theme
  isle:    'jULx2HuUYkY',  // Cha-La Head-Cha-La
  battle:  'kGsIUNkMmBY',  // DBZ Battle Theme
  victory: 'Y5cwBlPFq3w',  // DBZ Victory
  boss:    'kGsIUNkMmBY'   // Boss = Battle
};
...
// ══════════════════════════════════════════════════════
// FOND ANIMÉ DBZ (même système que manga-bg One Piece)
// Utilise Jikan anime 813 = Dragon Ball Z
// ══════════════════════════════════════════════════════
...
async function loadHistBgStrips() {
  ...
  try {
    ...
    for (var s = 0; s < 5; s++) {
      ...
      var all = slicePics.concat(slicePics); // double pour boucle
...
// Fallback : images statiques DBZ si Jikan rate-limite
function loadHistBgFallback() {
  var bg = document.getElementById('hist-bg');
  if (!bg || bg.children.length > 0) return;
  var imgs = [
    'assets/images/dbz/goku.png',
    'assets/images/dbz/vegeta.png',
    'assets/images/dbz/piccolo.png',
    'assets/images/dbz/gohan.png',
    'assets/images/dbz/trunks.png',
    'assets/images/dbz/krilin.png',
    'assets/images/dbz/bulma.png',
    'assets/images/dbz/yamcha.png'
  ];
  for (var s = 0; s < 5; s++) {
    var strip = document.createElement('div');
    strip.className = 'hist-bg-strip';
...
const ISLANDS_HISTOIRE = {
  ...
  1:{name:"ÎLE DE GOKU",charName:"Goku",color:"#f97316",topic:"L'Égypte Ancienne",level:"6ème",
    ...
    qs:[
      ...
      {q:"⚔️ BOSS — Freezer envahit l'Égypte !<br><em>Quel animal sacré représentait le dieu Anubis ?</em>",o:["Le crocodile","Le chacal","Le chat","L'ibis"],a:"Le chacal",exp:"Anubis, dieu de l'embaumement, était représenté avec une tête de chacal. Il guidait les âmes vers l'au-delà.",isBoss:true,bossName:"Freezer",bossImg:""}
    ]
  },
  ...
  2:{name:"ÎLE DE VEGETA",charName:"Vegeta",color:"#3b82f6",topic:"La Grèce Antique",level:"6ème",
    ...
    qs:[
      ...
      {q:"⚔️ BOSS — Cell s'empare d'Athènes !<br><em>Qui a fondé la première démocratie athénienne au VIe s. av. J.-C. ?</em>",o:["Périclès","Clisthène","Socrate","Alexandre"],a:"Clisthène",exp:"Clisthène (508 av. J.-C.) est le père de la démocratie athénienne. Périclès l'a ensuite perfectionnée.",isBoss:true,bossName:"Cell",bossImg:""}
    ]
  },
  ...
  4:{name:"ÎLE DE GOHAN",charName:"Gohan",color:"#a78bfa",topic:"Les Religions Monothéistes",level:"6ème",
    ...
    qs:[
      ...
      {q:"Quelle ville est sainte pour l'islam, lieu de naissance de Mahomet ?",o:["Jérusalem","Médine","La Mecque","Bagdad"],a:"La Mecque",exp:"La Mecque est la ville la plus sainte de l'islam. Les musulmans y font le pèlerinage (hajj)."},
  ...
  5:{name:"ÎLE DE TRUNKS",charName:"Trunks",color:"#8b5cf6",topic:"Le Moyen-Âge",level:"5ème",
    ...
    qs:[
      ...
      {q:"⚔️ BOSS — Beerus, Dieu de la Destruction, attaque !<br><em>En quelle année Guillaume le Conquérant conquiert-il l'Angleterre ?</em>",o:["1066","1096","1215","1337"],a:"1066",exp:"Guillaume le Conquérant bat Harold à la bataille d'Hastings (1066) et devient roi d'Angleterre.",isBoss:true,bossName:"Beerus",bossImg:""}
    ]
  },
  ...
  6:{name:"ÎLE DE KRILIN",charName:"Krilin",color:"#f59e0b",topic:"L'Islam et son Expansion",level:"5ème",
    ...
    qs:[
      ...
      {q:"⚔️ BOSS — Babidi menace Bagdad !<br><em>Comment s'appelle le livre de voyage d'Ibn Battouta ?</em>",o:["Les Mille et une Nuits","La Rihla","Les Croisades","Al-Qanun"],a:"La Rihla",exp:"'La Rihla' (le Voyage) d'Ibn Battouta raconte ses 28 ans de voyage à travers l'Afrique, l'Asie et l'Europe.",isBoss:true,bossName:"Babidi",bossImg:""}
    ]
  },
  ...
  7:{name:"ÎLE DE BULMA",charName:"Bulma",color:"#06b6d4",topic:"Chrétienté et Croisades",level:"5ème",
    ...
    qs:[
      ...
      {q:"⚔️ BOSS — Black Goku attaque Jérusalem !<br><em>En quelle année la première croisade prend-elle Jérusalem ?</em>",o:["1095","1099","1187","1291"],a:"1099",exp:"Jérusalem est prise par les croisés le 15 juillet 1099, 4 ans après le lancement de la 1ère croisade.",isBoss:true,bossName:"Black Goku",bossImg:""}
    ]
  },
...
const HIST_ISLE_INTRO = {
  1:{bg:'#1a0800',lines:['ÉGYPTE…','… ANCIENNE !!','Le Nil coule dans tes veines !'],kanji:'古代 !!',kanjiColor:'#f97316',bubble:"Kaio-Ken × 3 ! On va tout savoir sur l'Égypte !"},
...
function hist_corriger(n) {
  ...
  if(score===10) hist_playBGM('victory'); else hist_playBGM('map');
...
// ══════════════════════════════════════════════════════════════
// CINÉMATIQUE PLEIN ÉCRAN DBZ (avec image perso en grand)
// ══════════════════════════════════════════════════════════════
