...
function sfxCineDrum() {
  try {
    const c=getActx(),buf=c.createBuffer(1,c.sampleRate*.3,c.sampleRate),d=buf.getChannelData(0);
    for(let i=0;i<d.length;i++) d[i]=(Math.random()*2-1)*Math.exp(-i/(c.sampleRate*.05));
    const src=c.createBufferSource(); src.buffer=buf;
    const g=c.createGain(); g.gain.setValueAtTime(.9,c.currentTime); g.gain.exponentialRampToValueAtTime(.001,c.currentTime+.3);
...
function sfxCineRiser() {
  try {
    ...
    o.type='sawtooth'; o.frequency.setValueAtTime(80,c.currentTime); o.frequency.exponentialRampToValueAtTime(600,c.currentTime+.7);
    g.gain.setValueAtTime(.4,c.currentTime); g.gain.exponentialRampToValueAtTime(.001,c.currentTime+.8);
    o.connect(g); g.connect(c.destination); o.start(); o.stop(c.currentTime+.8);
  } catch(e) {}
}

function sfxCineVictory() {
  [0,.15,.3,.45].forEach((delay,i) => {
    try {
      const c=getActx(),o=c.createOscillator(),g=c.createGain(),notes=[523,659,784,1047];
      o.type='triangle'; o.frequency.value=notes[i];
      g.gain.setValueAtTime(.35,c.currentTime+delay); g.gain.exponentialRampToValueAtTime(.001,c.currentTime+delay+.35);
      o.connect(g); g.connect(c.destination); o.start(c.currentTime+delay); o.stop(c.currentTime+delay+.35);
    } catch(e) {}
  });
}
...
function sfxCineDefeat() {
  [0,.18,.36].forEach((delay,i) => {
    try {
      const c=getActx(),o=c.createOscillator(),g=c.createGain(),notes=[400,320,220];
      o.type='sawtooth'; o.frequency.value=notes[i];
      g.gain.setValueAtTime(.3,c.currentTime+delay); g.gain.exponentialRampToValueAtTime(.001,c.currentTime+delay+.5);
      o.connect(g); g.connect(c.destination); o.start(c.currentTime+delay); o.stop(c.currentTime+delay+.5);
    } catch(e) {}
  });
}
...
function showCine(panelsHTML, kanjiEl, bottomHTML, bgColor, particleType, callback) {
  ...
  // Bloquer boutons 2.8s
  document.querySelectorAll('#cineBottom .btn').forEach(b => { b.disabled=true; b.style.opacity='0.4'; });
  setTimeout(() => { document.querySelectorAll('#cineBottom .btn').forEach(b => { b.disabled=false; b.style.opacity='1'; }); }, 2800);
  // Kanji sur #cine-scene (overlay global)
  if (kanjiEl) {
